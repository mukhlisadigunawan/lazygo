...
func Run(inputs ...string) error {
	...
	err = utils.CreateEverythingExactly2(".lazygo/templates/controller/", obj.DriverName, fileRenamer, obj)
	if err != nil {
		return err
	}
...
func getHTTPClientTemplate(driverName, method string) ([]byte, error) {
	path := fmt.Sprintf(".lazygo/templates/controller/%s/domain_${domainname}/controller/${controllername}/~httpclient-%s._http", driverName, method)
	return os.ReadFile(path)
}

func getHandlerTemplate(driverName string) ([]byte, error) {
	path := fmt.Sprintf(".lazygo/templates/controller/%s/domain_${domainname}/controller/${controllername}/~handler._go", driverName)
	return os.ReadFile(path)
}
...
func getRouterRegisterTemplate(driverName, usecase string) ([]byte, error) {
	...
	switch {
	case strings.HasSuffix(usecaseLower, "getall"),
		strings.HasSuffix(usecaseLower, "getone"):
		method = "get"

	case strings.HasSuffix(usecaseLower, "update"):
		method = "put"

	case strings.HasSuffix(usecaseLower, "delete"):
		method = "delete"

	case strings.HasSuffix(usecaseLower, "create"):
		method = "post"
	}
	path := fmt.Sprintf(".lazygo/templates/controller/%s/domain_${domainname}/controller/${controllername}/~inject-router-register-%s._go", driverName, method)
	return os.ReadFile(path)
}
