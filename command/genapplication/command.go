...
func Run(inputs ...string) error {
	...
	{ //---------
		...
		// err = fmt.Errorf("usecase %s and %s", usecaseNames, existingUsecaseNames)
		// return err
		...
		for _, astFile := range pkg.Files {
			...
			for _, decl := range astFile.Decls {
				...
				ast.Inspect(decl, func(n ast.Node) bool {
					...
					SnakeCaseUsecase := utils.SnakeCase(s)
					if strings.HasSuffix(SnakeCaseUsecase, HandlerSuffix) {
						uc := SnakeCaseUsecase[:strings.LastIndex(SnakeCaseUsecase, HandlerSuffix)]
						uc = strings.TrimSuffix(uc, "_")
						res = append(res, uc)
					}
					return true
				})
